...
public interface StudentPsiRepository extends JpaRepository<StudentPsiEntity, String> {
    @Query(value = "SELECT * FROM STUD_PSI sp INNER JOIN TAB_POSTSEC tab ON tab.PSI_CODE=sp.PSI_CODE WHERE tab.TRANSMISSION_MODE = :transmissionMode AND sp.PSI_YEAR=:psiYear AND (:psiCodeProvided is null or tab.PSI_CODE in :psiCode)",nativeQuery = true)
