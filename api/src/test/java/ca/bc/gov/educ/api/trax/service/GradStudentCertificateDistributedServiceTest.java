...
@SpringBootTest
@ActiveProfiles("test")
public class GradStudentCertificateDistributedServiceTest {
    @Autowired
    private GradStudentCertificateDistributedService gradStudentCertificateDistributedService;
    ...
    private void testProcessEvent(String program, String studentStatus) throws JsonProcessingException {
        ...
        final var event = TestUtils.createEvent(EventType.STUDENT_CERTIFICATE_DISTRIBUTED.name(), request, eventRepository);
        this.gradStudentCertificateDistributedService.processEvent(request, event);
    }
